...
/**
 * One project: the screens full width (big enough to read a dashboard), then the story beneath in two
 * columns, what it is on the left and what I did on the right. With screenshots off, the story alone.
 */
function Case({ project }: { project: CaseStudy }) {
  ...
  return (
    ...
    <article className="group grid grid-cols-1 py-12 first:pt-0 md:py-14">
      {screens ? (
        // Extra room under a phone capture, which hangs below the frame.
        <div className={`relative mb-10 ${project.phone ? 'md:mb-20' : ''}`}>
          <Frame project={project} sizes="(min-width: 80rem) 75rem, 92vw" />
          {project.phone && (
            <img
              data-reveal="up"
              style={stagger(3)}
  ...
  return (
    <section id="work" aria-labelledby="work-title" className="dark bg-night text-snow">
      <div className="container-page py-24 md:py-32">
        ...
        {/* A hairline between projects, with the same room above and below each one. */}
        <div className="mt-12 divide-y divide-night-line md:mt-16">
          {cases.map((project) => (
            <Case key={project.name} project={project} />
          ))}
          <Builds />
        </div>
      </div>
